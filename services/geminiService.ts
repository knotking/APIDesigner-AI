...
export const generateCodeArtifact = async (
    ...
    switch (type) {
        ...
        case 'mock-server':
            ...
            break;
        case 'sdk-agent':
            typeDescription = "AI Agent using API SDK";
            specifics = `
            - Create an autonomous AI agent script that can interact with this API.
            - Use a popular agent framework for ${language} (e.g., LangChain, AutoGen, or raw LLM loop).
            - Assume a client SDK class named 'ApiClient' exists (as generated by the API Client option) or generate a mock one inline if needed for context.
            - Map the API's key endpoints to tools/functions the agent can call.
            - Include a main loop where the agent accepts a natural language goal and executes API calls to achieve it.
            - Provide a complete, runnable script that initializes the agent and runs a sample query.
            `;
            break;
        case 'documentation':
